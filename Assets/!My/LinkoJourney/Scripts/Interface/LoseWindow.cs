...
public class LoseWindow : MonoBehaviour
{
    ...
    public void Show(int score, int scoreAll, int level)
    {
        gameObject.SetActive(true);

        // Сначала масштабируем окно, а потом применяем Punch анимацию
        _frameWindow.localScale = Vector3.zero; // Начальный масштаб
        _frameWindow.DOScale(_scaleTarget, _scaleDuration).SetEase(Ease.Linear).OnComplete(() =>
        {
            _frameWindow.DOPunchScale(Vector3.one * _punchStrength, _punchDuration, _punchVibrato, _punchRandomness).SetEase(Ease.Linear);
        });
